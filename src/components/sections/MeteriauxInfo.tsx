...
import { FaCubes, FaTimes, FaPlusCircle, FaTrash } from "react-icons/fa";
...
const MateriauxInfo: React.FC = () => {
    ...
    const [materiauToDelete, setMateriauToDelete] = useState<number | null>(
        null
    );
    ...
    const handleDeleteMateriau = () => {
        if (materiauToDelete !== null) {
            setMateriaux((prevMateriaux) =>
                prevMateriaux.filter(
                    (materiau) => materiau.id !== materiauToDelete
                )
            );
            ...
            >
                <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <select
                        value={materiau.nom}
                        onChange={(e) =>
                            handleChange(materiau.id, "nom", e.target.value)
                        }
